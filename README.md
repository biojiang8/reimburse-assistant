...
- **MIT License**（见 `LICENSE`）：允许自由使用、修改、分发（含商用），
  但必须保留版权声明与许可声明。
- **署名要求**（见 `NOTICE`）：使用或修改本软件时须注明原作者
  biojiang8（BioJiang Lab），不得抹除原始出处；衍生作品建议标注
  "基于 reimburse-assistant 开发"及原仓库链接。
- 修改后**不强制开源**（如需"改后必须开源"请改用 GPLv3）。
